  ...
  return (
    <div>
          ...
          <SectionCard title="Coach Notes" icon={<IconNote className="h-5 w-5" />} delay={260} bodyCls="p-4">
            ...
            {(client.coachNotes ?? []).length === 0 ? (
              ...
              <ul className="mt-4 grid gap-2">
                {[...(client.coachNotes ?? [])].sort((a, b) => b.date.localeCompare(a.date)).map((n) => (
                    ...
                    {editingNote?.id === n.id ? (
                      ...
                      <div className="flex items-start gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-1 text-[10.5px] font-bold uppercase tracking-wider text-mist-500">{fmtDate(n.date)}</p>
                        </div>
                ...
                {history.map((s) => {
                  ...
                  return (
                    <li key={s.id} className={`flex items-center gap-2 rounded-lg border border-night-700 bg-night-800 px-2.5 py-2 text-xs ${isCurrent ? "ring-1 ring-volt-400/40" : ""}`}>
                      ...
                      <span className={`font-display text-[11px] font-bold ${SUB_STATE_META[si.state].text}`}>{si.state}</span>
                    </li>
                  );
                })}
              </ul>
      ...
      <PaymentFormModal
        ...
        defaultSubscriptionId={current?.id}
      ...
      <NutritionTargetsModal open={nutritionOpen} clientId={client.id} initial={client.nutritionTargets} onClose={() => setNutritionOpen(false)} />
        ...
        <div className="space-y-4 p-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-night-700 bg-night-800 p-3">
                <p className="text-[10px] font-bold uppercase tracking-wider text-mist-500">Mood</p>
                <div className="mt-1.5"><MoodDots mood={checkIn.mood} size="h-2 w-2" /></div>
              </div>
