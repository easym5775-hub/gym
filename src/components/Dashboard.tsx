      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-12">
              ...
              {alerts.slice(0, 6).map((a, i) => (
                <li key={a.key} className="rise" style={{ animationDelay: `${160 + i * 45}ms` }}>
                  <div className="group flex items-center gap-3.5 px-5 py-3 transition-colors hover:bg-night-800/60">
                    ...
                    <button
                      className={`${btnSecondary} ${btnSm} shrink-0 border-night-600 opacity-70 transition group-hover:border-volt-400/60 group-hover:text-volt-300 group-hover:opacity-100`}
        ...
        <section className="rise flex h-full flex-col rounded-xl border border-night-700 bg-night-850 lg:col-span-5" style={{ animationDelay: "170ms" }}>
          <header className="flex items-center gap-2.5 border-b border-night-700 px-5 py-3.5">
            ...
            <span className="font-display text-sm font-bold text-mist-500 tnum">{todaySessions.length}</span>
            {upcoming.length > 0 && (
  ...
  const inner = (
    <>
      <span className="flex items-center justify-between gap-2">
        <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-mist-500">{label}</span>
        <span className={`transition ${onClick ? "text-mist-500" : "text-night-500"}`}>{icon}</span>
      </span>
      <span className={`mt-2 block font-display text-[34px] font-bold leading-8 tnum ${tone === "warn" ? "text-warn-300" : "text-mist-100"}`}>
        {value}
        {unit && <span className="ms-1.5 text-sm font-semibold text-mist-500">{unit}</span>}
      </span>
      <span className="mt-1.5 block truncate text-[11px] font-semibold text-mist-500">{sub}</span>
    </>
  );
  const cls = `rise group relative border-night-700 p-4 transition-colors duration-200 max-lg:border-b max-lg:[&:nth-child(odd)]:border-e lg:border-e lg:last:border-e-0 lg:[&:nth-child(-n+4)]:border-b-0 ${className}`;
  if (onClick) {
    return (
      <button onClick={onClick} className={`${cls} cursor-pointer text-start hover:bg-night-800/70`} style={{ animationDelay: `${delay}ms` }}>
        {inner}
        <ArrowUpRight className="absolute end-3 top-3 h-3.5 w-3.5 text-mist-500 opacity-0 transition group-hover:opacity-100" />
        <span className="absolute bottom-0 left-0 h-0.5 w-0 bg-volt-400 transition-all duration-300 group-hover:w-full" />
      </button>
    );
  }
  return (
    <div className={`${cls} hover:bg-night-800/40`} style={{ animationDelay: `${delay}ms` }}>
      {inner}
    </div>
  );
}
...
function ScheduleRow({ s, delay, go }: { s: Session; delay: number; go: (v: CoachView, id?: string) => void }) {
  ...
  return (
    <li className="rise relative flex items-center gap-3.5 py-2.5" style={{ animationDelay: `${delay}ms` }}>
      <span
        className={`relative z-10 grid h-5 w-5 shrink-0 place-items-center rounded-full border-2 bg-night-850 ${
          isDone ? "border-moss-400 text-moss-300" : missed ? "border-danger-400/60 text-danger-300" : past ? "border-night-500 text-mist-500" : "border-volt-400 text-volt-300"
        }`}
      >
...
function BusinessPanel({
  ...
  return (
    <section className="rise flex h-full flex-col rounded-xl border border-night-700 bg-night-850 lg:col-span-5" style={{ animationDelay: "300ms" }}>
      ...
      <div className="mx-5 my-4 border-t border-night-700" />
...
function SubCount({
  ...
  return (
    ...
    >
      ...
      <span className={`mt-0.5 block font-display text-[26px] font-bold leading-7 tnum ${value > 0 ? tone : "text-mist-500"}`}>{value}</span>
    </button>
  );
}
